...
async function checkSession() {
    try {
        ...
        if (!data.loggedIn) { 
            window.location.href = '/cliente/login/'; // Redireciona se não estiver logado
        } else {
...
async function handleLogout() {
    await fetch('/cliente/sistema/php/logout.php', { method: 'POST' }); // Invalida a sessão no servidor
    window.location.href = '/cliente/login/'; // Redireciona para a página de login
}
...
function resetInactivityTimer() {
    ...
    inactivityTimer = setTimeout(() => {
        ...
        window.location.href = '/cliente/login/'; // Ao clicar em OK, redireciona
