...
if (isset($data['id'])) {
    ...
    if ($stmt = $conn->prepare($query)) {
        $stmt->bind_param("i", $storage);

        if ($stmt->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Storage excluído com sucesso']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Erro ao excluir o storage: ' . $stmt->error]);
        }
