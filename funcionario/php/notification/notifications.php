...
// Obtém os parâmetros de página e limite
...
// Prepara a consulta SQL para buscar as notificações relacionadas ao funcionário com o offset e limite
$sql = "SELECT id_notification, status
        FROM notificacoes
        WHERE id_funcionario = ? AND status IN ('nao_lido', 'lido')
        LIMIT ?, ?";

$stmt = $conn->prepare($sql);
$stmt->bind_param('iii', $id_funcionario, $offset, $limit);
$stmt->execute();
...
// Agora vamos contar o total de notificações para saber se há mais páginas
$total_sql = "SELECT COUNT(*) as total FROM notificacoes WHERE id_funcionario = ? AND status IN ('nao_lido', 'lido')";
$total_stmt = $conn->prepare($total_sql);
$total_stmt->bind_param('i', $id_funcionario);
$total_stmt->execute();
$total_result = $total_stmt->get_result();
$total_row = $total_result->fetch_assoc();
$totalNotifications = $total_row['total'];

// Retorna as notificações e o total de notificações em formato JSON
echo json_encode(['status' => 'success', 'notifications' => $response, 'totalNotifications' => $totalNotifications]);
